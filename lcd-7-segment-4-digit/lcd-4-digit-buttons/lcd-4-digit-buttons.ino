/* Connect the display and two buttons to arduino. Initially
 * the display should show 00:00. If left button is pressed
 * then it should increment left 00 section by one, if right
 * button is pressed, it should increment right 00 section by 
 * one. Pressing both buttons at the same times resets the display
 * to 00:00.
 */
...
const int buttonPin[] = {9,10,11,12};     // the numbers of the pushbutton pins

int buttonState[2] = {0,0};         // variable for reading the pushbutton status

int i=0, j=0; // digits 1-9 to display when button pressed
...
void loop()
{
  ...
  if(buttonsPressed[0] == 1 && buttonsPressed[3] == 1)
  {
    i = j = 0;
    display.showNumberDec(i,true,2,0);
    display.showNumberDec(j,true,2,2);    
    delay(1000);
  }
  ...
  buttonState[1] = digitalRead(buttonPin[3]);
  
  // check if the pushbutton on the keypad is pressed.
  // if it is, the buttonState is LOW:
    
  if (buttonState[0] == LOW && buttonPin[0] == 9) 
  {    
    ++i;
    if(i>23)
    {
  ...
  if (buttonState[1] == LOW && buttonPin[3] == 12) 
  {  
    ++j;
    if(j>59)
    {
